...
**Code layout** (`src/odometry/`): `common.py`, `step1_odometry.py`,
...
### B0 — Step 0 calibration ⬜ ⚠️ prerequisite
- Reconcile `ROTATE_90` with intrinsics; undistort fisheye (or fit a proper fisheye model).
- Validate camera↔LiDAR extrinsics by projection consistency (LiDAR edges vs image edges).
...
2. **Do Step 0 calibration first** (rotation + fisheye distortion) — hard prerequisite for
   B1/B3/E.
