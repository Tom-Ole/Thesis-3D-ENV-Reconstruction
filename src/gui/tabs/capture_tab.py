...
# Throttle live preview refresh so high capture rates don't choke the UI.
_PREVIEW_MIN_INTERVAL = 0.4
...
class CaptureTab(QWidget):
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        img_layout.addWidget(self.image_select_all)
        ...
        self.progress_label = QLabel("Images: 0 | Scans: 0 | Elapsed: 0.0s")
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        preview_box = QGroupBox("Latest image preview")
        preview_layout = QVBoxLayout(preview_box)
        self.preview_label = QLabel("No capture yet")
        self.preview_label.setAlignment(Qt.AlignCenter)
        self.preview_label.setMinimumHeight(280)
        self.preview_label.setStyleSheet("background:#202020; color:#888;")
        preview_layout.addWidget(self.preview_label)
        layout.addWidget(preview_box, stretch=1)

        summary_box = QGroupBox("Latest capture")
        summary_layout = QVBoxLayout(summary_box)
        self.summary_view = QPlainTextEdit()
        self.summary_view.setReadOnly(True)
        self.summary_view.setPlaceholderText("Capture details will appear here.")
        self.summary_view.setFixedHeight(180)
    ...
    def _on_start_capture(self) -> None:
        ...
        self.recording = True
        self.progress_label.setText("Images: 0 | Scans: 0 | Elapsed: 0.0s")
    ...
    def _show_summary(self, batch: CaptureBatchResult) -> None:
        lines = [
            f"Session: {batch.session_name}",
            f"Event id: {batch.event_id}",
            "",
        ]
        latest_image_preview: Optional[str] = None
        for res in batch.results:
            if res.capture_type == CAPTURE_TYPE_IMAGE:
                detail = f"{res.cols}x{res.rows}"
                latest_image_preview = res.preview_path
            else:
                detail = f"{res.num_points} points"
            lines.append(f"[{res.capture_type}] {res.source} ({detail})")
            lines.append(f"    {res.file_path}")
        self.summary_view.setPlainText("\n".join(lines))

        if latest_image_preview:
            self._update_preview(latest_image_preview)

    def _update_preview(self, path: str) -> None:
        now = time.monotonic()
        if now - self._last_preview_ts < _PREVIEW_MIN_INTERVAL:
            return
